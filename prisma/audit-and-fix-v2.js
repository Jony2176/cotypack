...
async function main() {
    // 1. Corregir imágenes (Agregar en lugar de pisar)
    ...
    console.log('Auditado: Cinta Acetato');

    await prisma.product.update({
        where: { slug: 'cinta-adhesiva-globos' },
        data: {
            images: JSON.stringify(['/images/products/cinta-adhesiva-globos-orig.jpg'])
        }
    });
    console.log('Auditado: Cinta Globos');

    // 2. Mapeo de Subcategorías (Asignar a categoría hija correspondiente)
    ...
    for (const p of products) {
        ...
        if (targetSlug) {
            ...
            if (sub) {
                ...
                console.log(`Producto "${p.name}" movido a subcategoría: ${sub.name}`);
            }
        }
    }
}
